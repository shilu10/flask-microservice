...
class ApiTesting(unittest.TestCase): 
    ...
    def test_creation_page_create_items_method_correct_content_type(self): 
        post_data = {"data": "correct content type"}
        content_type = "application/json"
        res = client.post("/creation-page/api/create-item", data = post_data, headers={'Content-Type': content_type})
        assert res.status_code == 200, "Create Method does'nt accepts the application json content type"
    
    def test_creation_page_create_items_method_correct_data(self): 
        post_data = {"title": "testing", "image": ""}
        content_type = "application/json"
        res = client.post("/creation-page/api/create-item", data = post_data, headers={'Content-Type': content_type})
        assert res.status_code == 200, "Create Method does'nt accepts the correct data"
    ...
    def test_creation_page_delete_method(self): 
        res = client.delete("/creation-page/api/items/delete/1")
        assert res.status_code == 200, "Delete method is not working"

    def test_creation_page_update_method(self): 
        update_data = {"title": "updated data"}
        res = client.put("/creation-page/api/items/update/1", data = update_data, headers = {"Content-Type": "application/json"})
        assert res.status_code == 200, "Update method is not working"
